...
pio.templates["health"] = go.layout.Template(
    layout=dict(
        ...
        legend=dict(bgcolor="rgba(0,0,0,0)", font=dict(color=C["text_sec"])),
...
<style>
  ...
  h1, h2, h3, h4, h5 {{ color: {C['text']} !important; }}
...
def _lifestyle_sleep(df: pd.DataFrame) -> None:
    ...
    if y_gluc:
        _dual_axis_chart(df, y_sleep, y_gluc, C["sleep"], C["glucose"], smooth)
    else:
...
def _lifestyle_activity(df: pd.DataFrame) -> None:
    ...
    if y_gluc:
        _dual_axis_chart(df, y_act, y_gluc, C["activity"], C["glucose"], smooth)
    else:
...
def _lifestyle_hrv_stress(df: pd.DataFrame) -> None:
    ...
    if stress_cols:
        ...
        _dual_axis_chart(df, "prev_night_hrv", y_stress, C["sleep"], C["warning"], smooth)

    if glucose_cols:
        st.markdown("#### HRV vs Glucose")
        y_gluc = st.selectbox("Glucose metric", glucose_cols, format_func=_label, key="hrv_gluc")
        _dual_axis_chart(df, "prev_night_hrv", y_gluc, C["sleep"], C["glucose"], smooth)
